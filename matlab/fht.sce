...
for i = 1:len_line(1)
    ...
    if(strstr(line(i), '`define Nx ') ~= '') then
        Nx = part(line(i), 12:15);
        [Nx, endstr] = strtod(Nx);
        flag_Nx = 1;
    end
    
    if(strstr(line(i), '`define Nh ') ~= '') then
        Nh = part(line(i), 12:15);
        [Nh, endstr] = strtod(Nh);
        flag_Nh = 1;
    end
...
if(~flag_N || ~flag_w_amp || ~flag_Nx || ~flag_Nh) then
...
elseif(strcmp(test, 'imp') == 0) then
    mprintf('\nimpulse test\n\n');
elseif(strcmp(test, 'signal') == 0) then
    mprintf('\nsignal test\n\n');
...
if(strcmp(test, 'imp') == 0) then
    file_imp = mopen(dir_imp, 'r'); // 'h.txt' must be autogenerated, cause 'Nh' defines in 'fht_defines.v'
elseif(strcmp(test, 'signal') == 0) then
    file_signal = mopen(dir_signal, 'r'); // 'x.txt' must be autogenerated, cause 'Nx' defines in 'fht_defines.v'
end
...
for i = 1:N_bank
    for j = 1:row
        ...
        elseif(strcmp(test, 'signal') == 0) then
            line = strtod(mgetl(file_signal, 1));
